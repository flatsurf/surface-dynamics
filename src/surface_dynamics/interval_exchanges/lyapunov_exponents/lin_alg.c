...
void
set_random_vectors(quad_cover * qcc)
...
/* warning: before calling this function call init_GS(dim) */
{
    ...
    if (qcc->nb_vectors == 1)
    {
        for (i = 0; i < qcc->nb_labels; i++)
            ...
            {
                if ((qcc->labels)[i].v[j] > 0)
                    norm += (qcc->labels)[i].v[j];
                else
                    norm -= (qcc->labels)[i].v[j];
            }
    }
    else
        orthogonalize_GS(qcc, NULL);
}

int
init_GS(size_t nb_vectors)
/* allocate scal and scal_new in order that it contains at least 2*nb_vectors elements */
{
    ...
    {
        while (scal_size < nb_vectors)
            scal_size = (scal_size * 3) / 2;
...
void
orthogonalize_GS(quad_cover * qcc, double *theta)
...
/* element at position (i,j,k): (qcc->labels)[j].v[i + nb_vectors*k];                          */
{
    ...
    {
        fprintf(stderr,
                "you must call init_GS before calling orthogonalize_GS.\n");
        exit(EXIT_FAILURE);
    }
    ...
    for (j = 0; j < qcc->nb_labels; ++j)
    {
        ...
        {
            scal_new[0] +=
                vv[0 + qcc->nb_vectors * k] * vv[1 + qcc->nb_vectors * k];
            sqnorm +=
                vv[0 + qcc->nb_vectors * k] * vv[0 + qcc->nb_vectors * k];
        }
    }
    ...
    for (i = 1; i < qcc->nb_vectors - 1; ++i)
    {
        ...
        for (j = 0; j < qcc->nb_labels; ++j)
        {
            ...
            for (k = 0; k < qcc->degree; ++k)
            {
                ...
                for (ii = 0; ii < i - 1; ++ii)
                    vv[i + qcc->nb_vectors * k] -=
                        scal[ii] * vv[ii + qcc->nb_vectors * k];

                /* subtract the part of the span of v_(i-1) */
                vv[i + qcc->nb_vectors * k] -=
                    c * vv[ii + qcc->nb_vectors * k];

                if (i + qcc->nb_vectors * k < 0
                    || i + qcc->nb_vectors * k >=
                    qcc->nb_vectors * qcc->degree)
                    printf("NO : %zu,  %zu, %zu\n", i, k,
                           i + qcc->nb_vectors * k);
                ...
                sqnorm +=
                    vv[i + qcc->nb_vectors * k] * vv[i + qcc->nb_vectors * k];
                for (ii = 0; ii <= i; ++ii)
                    scal_new[ii] +=
                        vv[ii + qcc->nb_vectors * k] * vv[i + 1 +
                                                          qcc->nb_vectors * k];
            }
        }
    }
    ...
    for (j = 0; j < qcc->nb_labels; ++j)
    {
        ...
        for (k = 0; k < qcc->degree; ++k)
        {
            for (ii = 0; ii < i - 1; ++ii)
                vv[i + qcc->nb_vectors * k] -=
                    scal_new[ii] * vv[ii + qcc->nb_vectors * k];

            if (i + qcc->nb_vectors * k < 0
                || i + qcc->nb_vectors * k >= qcc->nb_vectors * qcc->degree)
...
void
orthogonalize_iso(quad_cover * qcc, double *theta, size_t nb_char,
                  size_t * dimensions)
/* orthogonalization using Gram-Schmidt process                                                */
/* warning: this method should be called AFTER being sure that init_GS has been called         */
/* it theta is not NULL it is updated by 2*log of the diagonal from position 1 to nb_vectors   */
        ...
        if (dimensions[i_char] == 1)
        {
            ...
            for (j = 0; j < qcc->nb_labels; ++j)
            {
                ...
                for (k = 0; k < qcc->degree; ++k)
                    sqnorm +=
                        vv[sum_dimensions +
                           qcc->nb_vectors * k] * vv[sum_dimensions +
                                                     qcc->nb_vectors * k];
            }

            /* we renormalize v */
            if (theta != NULL)
                theta[sum_dimensions + 1] += log(sqnorm);
            norm = sqrt(sqnorm);
            for (j = 0; j < qcc->nb_labels; ++j)
            {
                vv = (qcc->labels)[j].v;
        ...
        if (dimensions[i_char] > 1)
        {
            ...
            for (j = 0; j < qcc->nb_labels; ++j)
            {
                ...
                for (k = 0; k < qcc->degree; ++k)
                {
                    scal_new[0] +=
                        vv[sum_dimensions +
                           qcc->nb_vectors * k] * vv[sum_dimensions + 1 +
                                                     qcc->nb_vectors * k];
                    sqnorm +=
                        vv[sum_dimensions +
                           qcc->nb_vectors * k] * vv[sum_dimensions +
            ...
            for (i = 1; i < dimensions[i_char] - 1; ++i)
            {
                ...
                c = scal[i - 1] / sqnorm;
                if (theta != NULL)
                    theta[sum_dimensions + i] += log(sqnorm);
                norm = sqrt(sqnorm);
                sqnorm = 0.;
                ...
                for (j = 0; j < qcc->nb_labels; ++j)
                {
                    ...
                    for (k = 0; k < qcc->degree; ++k)
                    {
                        /* subtract the part of the span of v_0, v_1, ..., v_(i-2) */
                        for (ii = 0; ii < i - 1; ++ii)
                            vv[sum_dimensions + i + qcc->nb_vectors * k] -=
                                scal[ii] * vv[sum_dimensions + ii +
                                              qcc->nb_vectors * k];

                        /* subtract the part of the span of v_(i-1) */
                        vv[sum_dimensions + i + qcc->nb_vectors * k] -=
                            c * vv[sum_dimensions + ii + qcc->nb_vectors * k];

                        /* normalize v_(i-1) */
                        vv[sum_dimensions + (i - 1) +
                           qcc->nb_vectors * k] /= norm;
                        ...
                        sqnorm +=
                            vv[sum_dimensions + i +
                               qcc->nb_vectors * k] * vv[sum_dimensions + i +
                                                         qcc->nb_vectors * k];
                        for (ii = 0; ii <= i; ++ii)
                            scal_new[ii] +=
                                vv[sum_dimensions + ii +
                                   qcc->nb_vectors * k] * vv[sum_dimensions +
                                                             i + 1 +
                                                             qcc->nb_vectors *
                                                             k];
            ...
            for (j = 0; j < qcc->nb_labels; ++j)
            {
                ...
                for (k = 0; k < qcc->degree; ++k)
                {
                    for (ii = 0; ii < i - 1; ++ii)
                        vv[sum_dimensions + i + qcc->nb_vectors * k] -=
                            scal_new[ii] * vv[sum_dimensions + ii +
                                              qcc->nb_vectors * k];

                    vv[sum_dimensions + i + qcc->nb_vectors * k] -=
                        c * vv[sum_dimensions + i - 1 + qcc->nb_vectors * k];
            ...
            /* we renormalize v_i */
            if (theta != NULL)
                theta[sum_dimensions + i + 1] += log(sqnorm);
            norm = sqrt(sqnorm);
            for (j = 0; j < qcc->nb_labels; ++j)
            {
                vv = (qcc->labels)[j].v;
...
void
check_orthogonality(quad_cover * qcc)
{
    double s;
    double *vv;
    size_t i1, i2, j, k;
    ...
    for (i1 = 0; i1 < qcc->nb_vectors; ++i1)
    {
        for (i2 = i1; i2 < qcc->nb_vectors; ++i2)
        {
            ...
            for (j = 0; j < qcc->nb_labels; ++j)
            {
                vv = (qcc->labels)[j].v;
                for (k = 0; k < qcc->degree; ++k)
                    s += vv[i1 + qcc->nb_vectors * k] * vv[i2 +
                                                           qcc->nb_vectors *
                                                           k];
            }
...
void
check_orthogonality_iso(quad_cover * qcc, size_t nb_char, size_t * dimensions)
{
    double s;
    double *vv;
    ...
    {
        fprintf(stderr,
                "Wrong isotypic decomposition : dimensions doesn't match\nSum of isotypic dimensions : %zu,   nb_vectors : %zu",
    ...
    for (i_char = 0; i_char < nb_char; ++i_char)
    {
        for (i1 = sum_dimensions; i1 < sum_dimensions + dimensions[i_char];
             ++i1)
        {
                        ...
                        for (deg_j = 0; deg_j < qcc->degree; ++deg_j)
                            res += (double) projections[i_char * (sq_size) +
                                                        ...
                                                        (deg_j *
                                                         qcc->nb_labels +
                                                         lab_j)] *
                                (qcc->labels)[lab_j].v[sum_dimensions + i_vec +
                                                       qcc->nb_vectors *
                                                       deg_j];
                    (qcc->v_buffer)[lab_i + qcc->nb_labels * deg_i] = res;
                    if (lab_i + qcc->nb_labels * deg_i >=
                        qcc->nb_labels * qcc->degree
                        || lab_i + qcc->nb_labels * deg_i < 0)
                        printf("ind : %zu, max : %zu\n",
                               lab_i + qcc->nb_labels * deg_i,
                               qcc->nb_labels * qcc->degree);
                }

            for (lab_i = 0; lab_i < qcc->nb_labels; ++lab_i)
                for (deg_i = 0; deg_i < qcc->degree; ++deg_i)
                    (qcc->labels)[lab_i].v[sum_dimensions + i_vec +
                                           qcc->nb_vectors * deg_i] =
                        (qcc->v_buffer)[lab_i + qcc->nb_labels * deg_i];
        }
        sum_dimensions += dimensions[i_char];
    }
}
    ...
    for (i_vec = 0; i_vec < qcc->nb_vectors; ++i_vec)
    {
        for (lab = 0; lab < qcc->nb_labels; ++lab)
            for (deg = 0; deg < qcc->degree; ++deg)
                sqnorm +=
                    (qcc->labels)[lab].v[i_vec +
                                         qcc->nb_vectors * deg] *
                    (qcc->labels)[lab].v[i_vec + qcc->nb_vectors * deg];
        if (sqnorm > max)
            max = sqnorm;
        sqnorm = 0;
    }
